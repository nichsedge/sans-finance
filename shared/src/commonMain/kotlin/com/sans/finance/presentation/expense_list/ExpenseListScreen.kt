...
fun ExpenseListScreen(
    ...
    val monthYear = remember(state.startDate) {
        if (state.startDate <= 0L) "All Time"
            ...
            DateTimeUtils.formatMonthYear(state.startDate)
        }
    }
    ...
    Scaffold(
                ...
                actions = {
                    IconButton(onClick = onSearchClick) {
                        Icon(
                            ...
                            contentDescription = stringResource(Res.string.search_expenses)
                        )
                    }
                    ...
                    DropdownMenu(
                        ...
                        DropdownMenuItem(
                            text = { Text(stringResource(Res.string.recurring_expenses)) },
                            onClick = {
                                showMenu = false
                        ...
                        DropdownMenuItem(
                            text = { Text(stringResource(Res.string.active_installments)) },
                            onClick = {
                                showMenu = false
                                onInstallmentsClick()
                            },
        ...
        Column(
            ...
            val todayMillis = remember {
                DateTimeUtils.getStartOfDay(DateTimeUtils.getNow())
            }
            ...
            LazyColumn(
                ...
                state.groupedExpenses.forEach { (date, expenses) ->
                    ...
                    item(key = "header-$date") {
                        val day = DateTimeUtils.getDayOfMonth(date).toString().padStart(2, '0')
                        val dayOfWeek = DateTimeUtils.getDayOfWeekName(date)
                        val monthYear = DateTimeUtils.formatMonthYear(date)
    ...
    if (showDatePicker) {
        val datePickerState = rememberDatePickerState(
            initialSelectedDateMillis = if (state.startDate > 0) state.startDate else com.sans.finance.core.util.DateTimeUtils.getNow()
        )
        DatePickerDialog(

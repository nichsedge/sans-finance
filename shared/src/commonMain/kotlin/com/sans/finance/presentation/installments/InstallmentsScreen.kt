    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        stringResource(Res.string.active_installments),
        ...
        Column(
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(24.dp)) {
                    Text(
                        stringResource(Res.string.total_monthly_due),
                    ...
                    Text(
                        stringResource(Res.string.total_remaining_balance) + ": " + com.sans.finance.core.util.CurrencyFormatter.formatAmount(
            ...
            if (currentList.isEmpty()) {
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    Text(
                        if (state.selectedTab == 0) stringResource(Res.string.no_active_installments) else "No history",
                    ...
                    items.forEach { monthlyItem ->
                        Row(
                            ...
                            Column {
                                Row(verticalAlignment = Alignment.CenterVertically) {
                                    Text(
                                        "${DateTimeUtils.formatMonthYear(monthlyItem.dueDate)} · Month ${monthlyItem.monthNumber}",
                                        style = MaterialTheme.typography.bodyMedium,
                                        fontWeight = FontWeight.Bold,
                                ...
                                Text(
                                    DateTimeUtils.formatDate(monthlyItem.dueDate),
                                    style = MaterialTheme.typography.bodySmall,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant
                                )
                            }
                            ...
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                Text(
                                    if (monthlyItem.status == "Paid") stringResource(Res.string.paid) else if (isOverdue) "Overdue" else stringResource(
                                        Res.string.pending
                                    ),
                                    style = MaterialTheme.typography.labelSmall,

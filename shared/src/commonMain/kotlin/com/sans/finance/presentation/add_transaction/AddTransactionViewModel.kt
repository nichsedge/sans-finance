...
class AddTransactionViewModel constructor(
    ...
    var selectedDate by mutableLongStateOf(Clock.System.now().toEpochMilliseconds())
    ...
    private fun saveTransaction(onSuccess: () -> Unit) {
        ...
        val nextDueDateVal = if (isRecurring) {
            when (recurrenceInterval) {
                "DAILY" -> com.sans.finance.core.util.DateTimeUtils.addDays(selectedDate, 1)
                "WEEKLY" -> com.sans.finance.core.util.DateTimeUtils.addDays(selectedDate, 7)
                "MONTHLY" -> com.sans.finance.core.util.DateTimeUtils.addMonths(selectedDate, 1)
                "YEARLY" -> com.sans.finance.core.util.DateTimeUtils.addMonths(selectedDate, 12)
                else -> selectedDate
            }
        } else null

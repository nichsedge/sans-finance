...
class ExpenseRepositoryImpl(
    ...
    private suspend inline fun <R> withDbTransaction(crossinline block: suspend () -> R): R {
    ...
    override suspend fun insertExpense(expense: Expense): Long = withDbTransaction {
    ...
    override suspend fun updateExpense(expense: Expense) = withDbTransaction {
            ...
            if (oldItem.status == "Pending" && expense.status == "Paid") {
                ...
                if (installment != null) {
                    val nextDate = installmentDao.getNextDueDateForInstallment(installment.id)
                        ?: com.sans.finance.core.util.DateTimeUtils.getNow() // Fallback
        ...
        accountDao.getAccountById(accountId)?.let { account ->
            accountDao.updateAccount(
                account.copy(
                    ...
                    updatedAt = com.sans.finance.core.util.DateTimeUtils.getNow()
                )
            )
        }
    }
    ...
    override suspend fun deleteExpense(expense: Expense) = withDbTransaction {
        if (expense.id >= INSTALLMENT_PAYMENT_ID_OFFSET) {
            val itemId = expense.id - INSTALLMENT_PAYMENT_ID_OFFSET
